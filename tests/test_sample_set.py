...
def test_todas_las_palabras_correctas(parity_afd: AFD) -> None:
    accept = ["", "11", "1001"]
    reject = ["1", "111"]
    result = verify_samples(parity_afd, accept, reject)
...
def test_falso_negativo_detectado(parity_afd: AFD) -> None:
    # "1001" tiene 2 unos (par), pero la marcamos como reject por error.
    result = verify_samples(parity_afd, [], ["1001"])
...
def test_falso_positivo_detectado(parity_afd: AFD) -> None:
    # "1" tiene 1 uno (impar), pero la marcamos como accept por error.
    result = verify_samples(parity_afd, ["1"], [])
    assert result.failed == 1
    verdict = result.mismatches[0]
...
def test_mezcla_de_aciertos_y_fallos(mod3_afd: AFD) -> None:
    ...
    result = verify_samples(mod3_afd, accept + rotas, reject)
...
def test_pretty_table_incluye_cabecera_y_simbolos(parity_afd: AFD) -> None:
    result = verify_samples(parity_afd, ["", "11"], ["1"])
