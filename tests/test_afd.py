...
def test_run_y_accepts(parity_dfa: AFD) -> None:
...
def test_run_paso_invalido(parity_dfa: AFD) -> None:
...
def test_transformacion_identidad_en_epsilon(parity_dfa: AFD) -> None:
    t = parity_dfa.transformation("")
    assert t == {q: q for q in parity_dfa.states}


def test_transformacion_0_es_identidad_en_paridad(parity_dfa: AFD) -> None:
    # En el AFD de paridad, leer un 0 no cambia el estado.
    t = parity_dfa.transformation("0")
    assert t == {"Par": "Par", "Impar": "Impar"}


def test_transformacion_1_invierte_paridad(parity_dfa: AFD) -> None:
...
def test_serializacion_roundtrip(parity_dfa: AFD, tmp_path) -> None:
    import json
...
def test_tabla_de_transiciones(parity_dfa: AFD) -> None:
