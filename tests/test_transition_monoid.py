...
def test_paridad_monoide_tiene_dos_elementos(parity_afd: AFD) -> None:
    M = TransitionMonoid(parity_afd)
...
def test_mod3_monoide_tiene_tres_elementos(mod3_afd: AFD) -> None:
    M = TransitionMonoid(mod3_afd)
...
def test_ends_with_01_monoide_no_es_grupo(ends_01_afd: AFD) -> None:
    M = TransitionMonoid(ends_01_afd)
...
def test_identidad_pertenece_y_es_neutro(parity_afd: AFD) -> None:
    M = TransitionMonoid(parity_afd)
    assert M.has_identity()
    for f in M.elements:
...
def test_cerradura(parity_afd: AFD, mod3_afd: AFD, ends_01_afd: AFD) -> None:
    for dfa in (parity_afd, mod3_afd, ends_01_afd):
        M = TransitionMonoid(dfa)
        assert M.is_closed()


def test_asociatividad(parity_afd: AFD, mod3_afd: AFD, ends_01_afd: AFD) -> None:
    for dfa in (parity_afd, mod3_afd, ends_01_afd):
        M = TransitionMonoid(dfa)
        assert M.is_associative()


def test_cota_superior_QexpQ(parity_afd: AFD, mod3_afd: AFD, ends_01_afd: AFD) -> None:
    # |M(A)| <= |Q|^|Q|.
    for dfa in (parity_afd, mod3_afd, ends_01_afd):
        M = TransitionMonoid(dfa)
        n = len(dfa.states)
...
def test_cayley_table_dimension(mod3_afd: AFD) -> None:
    M = TransitionMonoid(mod3_afd)
...
def test_cayley_table_consistente_con_then(mod3_afd: AFD) -> None:
    M = TransitionMonoid(mod3_afd)
    table = M.cayley_table()
...
def test_cayley_table_paridad_es_z2(parity_afd: AFD) -> None:
    M = TransitionMonoid(parity_afd)
...
def test_palabra_representante_minima(parity_afd: AFD) -> None:
    M = TransitionMonoid(parity_afd)
...
def test_transformacion_of_concuerda_con_dfa(mod3_afd: AFD) -> None:
    M = TransitionMonoid(mod3_afd)
    for w in ("", "0", "1", "10", "11", "101", "111"):
        esperado = Transformation(mod3_afd.transformation(w))
...
def test_palabras_representantes_son_shortlex(parity_afd: AFD) -> None:
...
def test_orden_de_columnas_cayley_es_consistente(mod3_afd: AFD) -> None:
